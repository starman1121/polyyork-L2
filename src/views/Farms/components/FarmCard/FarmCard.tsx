...
const StyledCardAccentSpecialYork = styled.div`
  background: linear-gradient(45deg,
  rgba(0, 255, 255, 1) 0%,
  rgba(0, 101, 255, 1) 10%,
  rgba(47, 255, 222, 1) 20%,
...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, bnbPrice, ethereum, account, wethPrice, aavePrice, dmaPrice, dogePrice, yorkPrice }) => {
  ...
  const totalValue: BigNumber = useMemo(() => {
    ...
    if (farm.quoteTokenSymbol === QuoteToken.DMA) {
      return dmaPrice.times(farm.lpTotalInQuoteToken)
    }
    if (farm.quoteTokenSymbol === QuoteToken.AAVE) {
      return aavePrice.times(farm.lpTotalInQuoteToken)
    }
    ...
    if (farm.quoteTokenSymbol === QuoteToken.YORK) {
      return yorkPrice.times(farm.lpTotalInQuoteToken)
    }
   
    return farm.lpTotalInQuoteToken
  }, [bnbPrice, cakePrice, farm.lpTotalInQuoteToken, farm.quoteTokenSymbol, wethPrice, dmaPrice, aavePrice, dogePrice, yorkPrice])
  ...
  return (
    <FCard>
      {farm.tokenSymbol === 'GOLDYORK' && <StyledCardAccent />}
      {farm.quoteTokenSymbol === 'CAKE' && <StyledCardAccent />}
